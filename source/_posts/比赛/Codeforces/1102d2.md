...
以顺时针为例，我们贪心地能加多少加多少，保证**不会让身后的桶水量增加**即可（**不保证不影响前方**），例如考虑第一个桶，序列h={3,4,2,6,1,5}，我们顺时针看（正向），得到的的序列为{0,4,4,6,6,6}，实际上就是转化为一个**单调不减序列**，也可看做单调栈。逆时针同理。
